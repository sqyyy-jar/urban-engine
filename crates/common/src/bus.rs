...
//! This module contains an instruction bus trait for ISA version `1.5.0-pre`.
...
pub trait InstructionBus {
    ...
    fn process(&mut self, insn: u32) {
        match insn {
            ...
            L0_BRANCH_ZR..=END_L0_BRANCH_ZR => self.l0_branch_zr(insn),
            L0_BRANCH_NZ..=END_L0_BRANCH_NZ => self.l0_branch_nz(insn),
            L0_LEA..=END_L0_LEA => self.l0_lea(insn),
            ...
            L4_ZERO..=END_L4_ZERO => self.l4_zero(insn),
            L4_DBG..=END_L4_DBG => self.l4_dbg(insn),
            L4_INC..=END_L4_INC => self.l4_inc(insn),
            L5_NOP..=END_L5_NOP => self.l5_nop(insn),
            L5_HALT..=END_L5_HALT => self.l5_halt(insn),
            L5_RET..=END_L5_RET => self.l5_ret(insn),
            _ => self.unknown(insn),
        }
    }
}
